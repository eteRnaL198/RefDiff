 ...
 * <ul>
 * <li>Find partition id using the distributed object name of cache as a
 * key.</li>
 * <li>Send the <code>CacheCreateConfigOperation</code> operation to the
 * calculated partition which will force all
 * clusters to be single threaded.</li>
 * <li>{@link CacheService#createCacheConfigIfAbsent(com.hazelcast.config.CacheConfig)}
 * is called.</li>
 * </ul>
 * </p>
 * <p>
 * This operation's purpose is to pass the required parameters into
 * {@link CacheService#createCacheConfigIfAbsent(com.hazelcast.config.CacheConfig)}.
 * </p>
 */
public class CacheCreateConfigOperation
  ...
  private void postponeReturnResponse() {
    // If config already exists or it's local-only created then return response
    // immediately.
