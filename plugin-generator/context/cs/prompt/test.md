...
To generate parser tests for the Typescript language, first, learn the language-independent common testing patterns from the following JavaScript and Ruby test implementations.

- Overview:
    - Test Target Code: Code that covers various function definitions, including edge cases.
...
# Task (Generation Target)
Apply the test patterns you learned above to generate a new Java JUnit test method that analyzes the following new ts test target file.

Generation Target: ts
- Overview:
    - Test Target Code: ts language code containing classes and functions.
    - Test Implementation (TestParser.java): A JUnit test that parses the aforementioned ts file and verifies that classes, methods, and function definitions are correctly recognized as CST nodes.
- File Reference:
    - Test Target Code:
        - @plugin-maker/context/ts/src/func.ts
    - Test Target Ctags:
        - @plugin-maker/context/ts/tags/tags-func.json
    - Test Implementation: @refdiff-universal/src/test/java/refdiff/parsers/universal/ts/TestTsParser.java

# Output Requirements
- Generation Language: The output test code must be Java.
- Inference and Application:
- Infer the language's node types and use them, such as NodeTypes.FUNCTION.
- Accurately map the code structure (packages, functions, type definitions, etc.) to ExpectedNode records.
- Naming Convention: The method name should clearly describe the feature of the language being tested, for example, shouldParseFunctionsCorrectly.
...
- Format: Output the generated code as a complete Java method, suitable for being placed in a new refdiff/parsers/universal/ts/TestTsParser.java file.
