...
# Premise
- **Target Grammar:** `tree-sitter-typescript`
...
- **Usage:** The generated query string will be used directly in a TypeScript binding (e.g., in `Query.create(language, queryString)`).
...
corresponding parsed output AST:
- @plugin-maker/context/ts/ast/ast-func.txt
...
- For TypeScript Binding: The generated output must be plain text, ready to be passed directly into a TypeScript method like Query.create(). (Do NOT escape it for a TypeScript string literal, e.g., "...").
- Accuracy: You must use the precise node names from the tree-sitter-typescript grammar (e.g., class_declaration, formal_parameters, block).